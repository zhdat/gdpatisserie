...
async function getRevenueData() {
  ...
  orders.forEach((order) => {
    const dayName = order.createdAt.toLocaleDateString("fr-FR", {
      weekday: "short",
    });
  ...
  return Object.entries(groupedRevenue).map(([name, total]) => ({
    name,
    total,
  }));
}
...
export default async function AdminDashboardPage() {
  const [pendingOrdersCount, productsCount, revenueData, totalRevenue] =
    await Promise.all([
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Link href="/admin/orders">
          <Card className="hover:shadow-lg transition-all hover:-translate-y-1 cursor-pointer border-amber-200 bg-amber-50/50 h-full">
        ...
        <a href="/" target="_blank" rel="noopener noreferrer">
          <Card className="hover:shadow-lg transition-all hover:-translate-y-1 cursor-pointer bg-slate-900 text-white border-slate-800 h-full">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">Voir le site</div>
      ...
      <div className="grid grid-cols-1">
        <Card>
          <CardHeader>
            <CardTitle>
              Évolution du Chiffre d&apos;affaires (7 jours)
            </CardTitle>
          </CardHeader>
