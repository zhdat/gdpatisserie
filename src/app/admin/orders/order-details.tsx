  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-md bg-white">
        <DialogHeader>
          <DialogTitle>Commande #{order.idString.slice(-4).toUpperCase()}</DialogTitle>
          <DialogDescription>
            Détails complets à préparer
          </DialogDescription>
        </DialogHeader>
              ...
              {order.items.map((item) => (
                <li key={item.id} className="flex justify-between items-center text-sm">
                  <div className="flex items-center gap-3">
                                <span
                                  className="bg-slate-900 text-white font-bold px-2 py-1 rounded text-xs">
                                    x{item.quantity}
                                </span>
